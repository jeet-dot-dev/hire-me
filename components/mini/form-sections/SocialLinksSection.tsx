...
import { Plus, Trash2, Link, Github, Linkedin, Globe, Twitter } from 'lucide-react';
...
export function SocialLinksSection({ socialLinks, onSocialLinksChange }: SocialLinksSectionProps) {
  const platforms = [
    ...
    <Card className="bg-card border-border">
      ...
      <CardContent className="space-y-6">
        {socialLinks.map((link, index) => {
          ...
          return (
            <div key={link.id} className="p-4 border border-border rounded-lg bg-muted/20">
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-2">
                  <IconComponent className="w-4 h-4 text-muted-foreground" />
                  <h4 className="font-medium text-foreground">Social Link {index + 1}</h4>
                </div>
                <Button
                  type="button"
                  variant="ghost"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Select
                    value={link.platform}
                    onValueChange={(value) =>
                      updateSocialLink(link.id, { platform: value as SocialLink['platform'] })
                    }
                  >
